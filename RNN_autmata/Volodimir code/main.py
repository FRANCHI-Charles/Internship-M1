...
def net_train(net, train_loader, parameters, dtype,  device):
    ...
    for _ in range(num_epochs):
        for _, (images, labels) in enumerate(train_loader):
            ...
            outputs = net(images)
            loss = criterion(outputs, labels) #+ torch.exp(reg_param*(net.get_beta()-torch.sqrt(1-1/Norm)))
...
def init_net(parameters):
    model = md.WcustomSRN(hidden_dim = parameters.get('hidden_size',128),input_dim=input_length, output_dim=2,epsilon = parameters.get('epsilon', 0.1),seq_length=seq_leng,device=device,dtyp=32)
    return model 
...
def evaluate_saturate(
    ...
    # print("This net got the following performance:" + str(res/(reg+10**(-4))))
    out = res/(reg+10**(-4))
...
for j in range(1,4):
    ...
    model = md.WcustomSRN(hidden_dim = bp.get('hidden_size',128),input_dim=input_length, output_dim=2,epsilon = bp.get('epsilon', 0.1),seq_length=seq_leng,device=device,dtyp=32)
    criterion = nn.CrossEntropyLoss()
...
with open('perf_dict_contract.pickle', 'wb') as handle:
